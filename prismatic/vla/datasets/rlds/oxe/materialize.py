...
def make_oxe_dataset_kwargs(
    ...
    if dataset_kwargs["action_encoding"] not in [ActionEncoding.EEF_POS, ActionEncoding.EEF_R6, ActionEncoding.DRONE_VEL]:
        raise ValueError(f"Cannot load `{dataset_name}`; only EEF_POS & EEF_R6 actions supported!")

    if dataset_kwargs["action_encoding"] is ActionEncoding.DRONE_VEL:
	#Change vector size of action data to size of data
        dataset_kwargs["absolute_action_mask"] = [False] * 1
        dataset_kwargs["action_normalization_mask"] = [True] * 1
